...
@brief Primary Goal: Neural Network to map visual data to the ground truth offsets.
"""
...
def run_epoch(model: nn.Module, criterion: nn.Module, optimizer, train_loader, train_transform: v2.Compose, epoch: int):
    ...
    for batched_stereo_sample in train_loader:
        batched_stereo_sample.move_to(device)
        optimizer.zero_grad()
        #TODO: now what would be ideal is that I add stereo sample
        # to the pytree and just call transform on it, but I digress
        batched_stereo_sample.transform(train_transform)
        pred_offsets = model(batched_stereo_sample)
        loss = criterion(pred_offsets, batched_stereo_sample.batched_offset)
