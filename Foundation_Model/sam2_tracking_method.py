...
class CameraStreamer(threading.Thread):
    ...
    def run(self):
        ...
        if self.cam_index > 0:
            init_params.set_from_camera_id(self.cam_index)
        ...
        img_res = cam_info.camera_configuration.resolution
        print(f"ZED camera opened: {img_res.width}x{img_res.height}")
        if min(img_res.width, img_res.height) < 512:
            print("WARNING: resolution below 512 — consider HD720 or higher")

        video_path = time.strftime("dinov2_track_%Y%m%d_%H%M%S.svo2")
        rec_params = sl.RecordingParameters()
        rec_params.video_filename = video_path
        rec_params.compression_mode = sl.SVO_COMPRESSION_MODE.H264
        rec_status = zed.enable_recording(rec_params)
        if rec_status != sl.ERROR_CODE.SUCCESS:
            print(f"Recording init failed: {rec_status}")
            video_path = None
        else:
