...
class StereoSample:
    ...
    def collate(samples: List["StereoSample"]):
        return BatchedStereoSample(*samples)

    def transform(self, transforms: v2.Compose | Callable[[torch.Tensor, torch.Tensor], torch.Tensor]):
        """
        transforms MUST take two images as parameters, AND take arbitrary batch dimensions.
        """

        with tv_tensors.set_return_type("TVTensor"):
            self.left_img, self.right_img = transforms(self.left_img, self.right_img)
        return self
...
class BatchedStereoSample:
    def __init__(self, *stereo_samples: StereoSample):
        for f in fields(StereoSample):
            values = [getattr(s, f.name) for s in stereo_samples]
    ...
    def transform(self, transform: v2.Compose):
        with tv_tensors.set_return_type("TVTensor"):
            self.batched_left_img, self.batched_right_img = transform(self.batched_left_img, self.batched_right_img)
        return self
...
class LossInfoGpu:
    ...
    gt: torch.Tensor
    original_image_paths: Tuple[str, str]
